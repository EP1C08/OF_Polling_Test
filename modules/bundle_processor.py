...
def is_bundle(message) -> bool:
    ...
    Args:
        message: MessageModel object

    Returns:
        True if message is a bundle
    """
    has_media = message.media_count and message.media_count > 0
    is_paid_content = message.price and message.price > 0
    is_mass = message.is_mass_message()

    # Bundle if: (has media AND paid) OR (multiple media) OR (mass message with media)
    return has_media and (is_paid_content or message.media_count > 1 or is_mass)
...
def process_bundle_from_message(
    ...
    """
    # Extract media IDs
    media_ids = [str(media.get('id', '')) for media in message.media if media.get('id')]
    ...
    # Count media types
    photo_count, video_count, audio_count = extract_media_counts(message.media)
    ...
    bundle_data = {
        ...
        'total_price': float(message.price) if message.price else 0.0,
        'media_count': message.media_count or len(media_ids),
        'photo_count': photo_count,
        'video_count': video_count,
        'audio_count': audio_count,
        'is_mass_message': message.is_mass_message(),
        'queue_id': str(message.queue_id) if hasattr(message, 'queue_id') and message.queue_id else '',
        'name': bundle_name,
    ...
    fan_interaction = {
        ...
        'fan_user_id': fan_id,
        'message_id': str(message.id),
        'sent_at': message.created_at.isoformat() if isinstance(message.created_at, datetime) else str(message.created_at),
        'is_purchased': message.canPurchase is False if hasattr(message, 'canPurchase') else False,  # If can't purchase, already purchased
